...
public class CommentDao extends AbstractMFlixDao {
  ...
  public boolean updateComment(String commentId, String text, String email) {
	...
	if (comment.getEmail().equals(email)) {
		try {			
			comment = commentCollection.findOneAndUpdate(
					Filters.and(
   ...
   */
	public boolean deleteComment(String commentId, String email) {
		...
		try {
			comment = commentCollection.findOneAndDelete(filter);
		} catch(MongoWriteException wrEx) {
			throw new IncorrectDaoOperation("Some error occurred while updating comment"+wrEx.getError().getCategory());
		}
		return comment != null;
  ...
  public List<Critic> mostActiveCommenters() {
    ...
    commentCollection.withReadConcern(ReadConcern.MAJORITY).aggregate(
    		Arrays.asList(
    			    Aggregates.group("$email", Accumulators.sum("count", 1)),
    				Aggregates.sort(Sorts.descending("count")),
    				Aggregates.limit(20)
    		), Critic.class).iterator().forEachRemaining(mostActive::add);
    return mostActive;
  }
}
